...
from environments.rooms_envs import USE_SHAPED_REWARDS
...
def create_maze_env(env_name=None, top_down_view=False, max_timesteps=0):
  # rooms environments
  if env_name.startswith('Rooms'):
    env_num = int(env_name[5])
    grid_params = GRID_PARAMS_LIST[env_num]
    start_state = grid_params.make_abstract_state(grid_params.edges[0], is_start=True)
    goal_state = grid_params.make_abstract_state(grid_params.edges[-1])
    basic_rooms_env = RoomsEnv(grid_params, start_state, goal_state, goal_state, max_timesteps=max_timesteps,
                               use_shaped_rewards=USE_SHAPED_REWARDS[env_num])
    basic_rooms_env.reset()
    wrapped_env = gym_wrapper.GymWrapper(basic_rooms_env)
    return wrapped_env
