...
class TD3(OffPolicyRLModel):
    ...
    :param policy: (TD3Policy or str) The policy model to use
                    (MlpPolicy, CnnPolicy, LnMlpPolicy, ...)
    ...
    :param tau: (float) the soft update coefficient
                ("polyak update" of the target networks, between 0 and 1)
    :param policy_delay: (int) Policy and target networks will only be updated
                            once every policy_delay steps
        per training steps. The Q values will be updated policy_delay more often
                            (update every training step).
    :param action_noise: (ActionNoise) the action noise type.
                        Cf DDPG for the different action noise type.
    ...
    :param random_exploration: (float) Probability of taking a random action
                                (as in an epsilon-greedy strategy)
    ...
    :param _init_setup_model: (bool) Whether or not to build the network at
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate_pi=1e-4, learning_rate_qf=1e-3,
                 buffer_size=50000,
        ...
        self.learning_rate_qf = learning_rate_qf
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("input", reuse=False):
                    ...
                    self.policy_tf = self.policy(self.sess, self.observation_space,
                                                 self.action_space, **self.policy_kwargs)
                    self.target_policy_tf = self.policy(self.sess, self.observation_space,
                                                        self.action_space, **self.policy_kwargs)
                    ...
                    self.actions_ph = tf.placeholder(tf.float32, shape=(None,)
                                                     + self.action_space.shape,
                                                     name='actions')
                    self.learning_rate_pi_ph = tf.placeholder(
                        tf.float32, [], name="learning_rate_pi_ph")
                    self.learning_rate_qf_ph = tf.placeholder(
                        tf.float32, [], name="learning_rate_qf_ph")
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    policy_optimizer = tf.train.AdamOptimizer(
                        learning_rate=self.learning_rate_pi_ph)
                    ...
                    qvalues_optimizer = tf.train.AdamOptimizer(
                        learning_rate=self.learning_rate_qf_ph)
                    ...
                    tf.summary.scalar('learning_rate_qf', tf.reduce_mean(self.learning_rate_qf_ph))
    ...
    def _train_step(self, step, writer, learning_rate_pi, learning_rate_qf, update_policy):
        ...
        feed_dict = {
            ...
            self.learning_rate_qf_ph: learning_rate_qf
        }
    ...
    def learn(self, total_timesteps, callback=None,
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log,
            ...
            self.learning_rate_qf = get_schedule_fn(self.learning_rate_qf)
            # Initial learning rate
            current_lr_pi = self.learning_rate_pi(1)
            current_lr_qf = self.learning_rate_qf(1)
            ...
            for step in range(total_timesteps):
                ...
                if (self.num_timesteps < self.learning_starts or
                        np.random.rand() < self.random_exploration):
                    # actions sampled from action space are from range specific to the environment
                    # but algorithm operates on tanh-squashed actions tf simple scaling is used
                ...
                # Only stop training if return value is False, not when it is None. This is for
                # backwards compatibility with callbacks that have no return statement.
                ...
                if self.num_timesteps % self.train_freq == 0:
                    ...
                    for grad_step in range(self.gradient_steps):
                        ...
                        current_lr_qf = self.learning_rate_qf(frac)
                        ...
                        mb_infos_vals.append(
                            self._train_step(step, writer, current_lr_pi, current_lr_qf,
                ...
                if done:
                    print('Reward at episode {}: {}'.format(
                        len(episode_rewards), episode_rewards[-1]))
                    if self.action_noise is not None:
                        self.action_noise.reset()
                ...
                if (self.verbose >= 1 and done and log_interval is not None
                    ...
                    logger.logkv("current_lr_pi", current_lr_pi)
                    logger.logkv("current_lr_qf", current_lr_qf)
    ...
    def save(self, save_path, cloudpickle=False):
        data = {
            ...
            "learning_rate_qf": self.learning_rate_qf,
