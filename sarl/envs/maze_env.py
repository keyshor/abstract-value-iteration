...
from sarl.envs import maze_env_utils
...
class MazeEnv(gym.Env):
    ...
    def __init__(
        ...
        self._init_positions = [
            (x - torso_x, y - torso_y)
            for x, y in self._find_all_robots()]  # distances between all robots and current robot
        ...
        self.wrapped_env = model_cls(*args, start_region=start_region, maze_id=maze_id,
    ...
    def get_top_down_view(self):
        ...
        # Draw movable blocks.
        for block_name, _ in self.movable_blocks:
    ...
    def step(self, action):
        ...
        if self._manual_collision:
            ...
            if self._is_in_collision(new_pos):
                ...
                # incentivices crashing! TODO: fix this if using PointEnv
        next_obs = self._get_obs()
        done = done or (self.t > self._max_timesteps)
        reward = 0
        if self.goal_region.contains(next_obs) and \
                (not self.goal_region.contains(old_obs)):
            reward = 100
        if self.final_region.contains(next_obs) and \
                (not self.start_region.contains(next_obs)):
            done = True
        if self.use_shaped_reward:
            reward = self.goal_region.reward(next_obs)
    ...
    def set_start_region(self, start_region):
        self.start_region = start_region
        self.wrapped_env.set_start_region(start_region)

    def set_goal_region(self, goal_region):
        self.goal_region = goal_region

    def set_start_state(self, start_state):
        self.wrapped_env.set_start_state(start_state)
